...
public class YunSingUp extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (nikeName != null && mail != null && password != null
				&& yzCode != null && !yzCode.isEmpty()) {
			System.out.println("name" + nikeName + "mail" + mail + "pass"
					+ password + "ycode" + yzCode);
					...
					if (mresult.next()) {
						...
						if (mresult.next()) {
							...
							if (sCode != null) {
								...
								if (intCode == yCode) {
									String insertSql = "insert into users (mail,nikename,head_url,password,registration_time) values('"
											+ mail
											+ "','"
											+ nikeName
											+ "',"
		...
		System.out.println("注册账号为：" + mail + "||状态为：" + resCoad + "||时间："
				+ sm.format(new Date()));
		out.print(sendMsg);
		out.flush();
		out.close();
